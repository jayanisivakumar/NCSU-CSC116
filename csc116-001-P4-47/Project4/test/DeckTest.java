...
public class DeckTest {
    ...
    /**
     * Tests nextCard by calling the nextCard() method multiple times
     * and ensures that the next nextCard() call returns the correct Card .
     */
    @Test
    public void testNextCardB() {
        // Call nextCard() multiple times
        Card firstCard = deck.nextCard();
        Card secondCard = deck.nextCard();
        Card thirdCard = deck.nextCard();
        Card fourthCard = deck.nextCard();

        // Test that the next nextCard() call returns the correct Card
        assertEquals(new Card('c', 2), firstCard, "Test first card");
        assertEquals(new Card('c', 3), secondCard, "Test second card");
        assertEquals(new Card('c', 4), thirdCard, "Test third card");
        assertEquals(new Card('c', 5), fourthCard, "Test fourth card");
    }
    ...
    /**
     * Test getNext method for initial value.
     */
    @Test
    ...
    /**
     * Test getNext after multiple nextCard calls.
     */
    @Test
    public void testGetNextB() {
        // Call nextCard() method multiple times
        deck.nextCard();
        deck.nextCard();
        deck.nextCard();
    
        // Test that getNext() returns the correct value
        assertEquals(3, deck.getNext(), "Test value of next after calling"
            + "nextCard() multiple times");
    }    
    ...
    /**
     * Test equals with equal cards arrays but different next values.
     */
    @Test
    public void testEqualsB() {
        Deck deck2 = new Deck();
        deck2.nextCard();
        assertFalse(deck.equals(deck2), "Test Decks with equal cards"
            + "arrays but different next values");
    }

    /**
     * Test equals with equal next but unequal cards arrays.
     */
    @Test
    public void testEqualsC() {
        Deck deck2 = new Deck();
        deck2.shuffle();
        assertFalse(deck.equals(deck2), "Test Decks with equal next but" 
            + "unequal cards arrays");      
    }    
